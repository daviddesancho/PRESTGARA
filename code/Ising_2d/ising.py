...
class Ising_lattice:
   ...
   The periodic identification is site [i,j] = site [i+m*N,j+l*N],
   ...
   cond_spin_flip(i,j,T) : flips (i,j)^th spin subject to the Metropolis-Hastings, conditions: 
                              (1) always if dE < 0
                              (2) with probability exp^(-dE/T) if dE>0, 
								(Boltzman's constant k=1 here)
                              Returns dE
   query: 
   ------
   ...
   plot(): plots the spin array using matplotlib
